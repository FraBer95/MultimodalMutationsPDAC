...
if __name__ == "__main__":
    ...
    if local_XAI:
        #samples_data = pd.read_csv(r'E:\Users\Berloco\PycharmProjects\CLAM\heatmaps\process_lists.csv')
        #sample_list = samples_data['slide_id'].str.slice(stop=9).to_list()
        sample_dict = { "C3N-03190-22" : "C3N-03190-02",
               "C3N-02585-22":"C3N-02585-02",
                "C3L-01598-22" : "C3L-01598-02",
                "C3L-00277-23":"C3L-00277-03"}
        sample_list = list(set(sample_dict.values()))
    ...
    for gene, model_name in gene_model.items():
            ...
            if Flag_autoencoder:
                ...
                result_path = f'./results_updated/AE'

            else:
                print("Loading DEG data and ML Model...")
                dim = ""
                path_to_ML_model = f'./log_voting/DEG/{gene}/{model_name}'
                df = pd.read_csv(f'../datasets/test_set/NEW_DEG/DEG_{gene}.csv')
                df['case_id'] = df['new_case_id']
                ...
                result_path = f'./results_updated/DEG_new'
