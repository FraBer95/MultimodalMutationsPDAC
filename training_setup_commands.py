...
for gene in genes:
    ...
    if Split:
        ...
        subprocess.run(["/home/berloco/anaconda3/envs/clam_uni/bin/python", 'create_splits_seq.py', '--task',  'task_3_wt_vs_mut', '--seed', '1',
                        '--label_frac', '1',  '--k', '10', '--target', '{}'.format(gene), '--csv_path', './dataset_csv/tcga_csv', '--target', gene])
    ...
    if Train:
        for f_extractor in f_extractors:
            for size in model_size:
                ...
                subprocess.run(["/home/berloco/anaconda3/envs/clam_uni/bin/python", "main.py", "--drop_out",  "--lr", "1e-5",  "--weighted_sample", "--bag_loss", "ce",
                            "--task", "task_3_HE_vs_mut", "--model_type", "clam_sb", "--data_root_dir", "/mnt/d/Datasets/Pancreas_Features",
                             "--split_dir", "tcga/task_3_wt_vs_mut_100_{}".format(gene), "--exp_code", "train_tcga_x20_{}_{}".format(gene, f_extractor),
                             "--early_stopping", "--csv_path", "./dataset_csv/tcga_csv", "--model_size", size, "--results_dir", "./results/mut_TCGA_model_{}_{}".format(size, f_extractor),
        ...
        for f_extractor in f_extractors:
            for size in model_size:
                ...
                subprocess.run(["/home/berloco/anaconda3/envs/clam_uni/bin/python", "eval.py",  "--data_root_dir", "/mnt/d/Datasets/Pancreas_Features",  "--results_dir", "./results/mut_TCGA_model_{}_{}".format(size, f_extractor),
                                ...
                                "--save_exp_code", "task_MUT_tcga_{}_{}".format(gene, size), "--task",  "task_3_wt_vs_mut", "--csv_path", "{}".format(gene),
