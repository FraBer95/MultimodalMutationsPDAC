...
def training_models(X_train, y_train, save_path):
    ...
    for classifier in classifiers:  #for each classifier
        ...
        if tuning: #if true do the fine tuning according model param grid
            ...
            if classifier_name == rf_classifier.__class__.__name__:
                ...
                X_train = X_train
                y_train = y_train
                grid = RandomizedSearchCV(estimator=xgb_classifier, param_distributions=param_grid_xgb, cv=5, scoring='roc_auc')
        ...
        performance_list.append({"Model": classifier_name, "AUROC": avg_auroc, "AUPRC": avg_auprc})
    ...
    avg_perf.to_csv(os.path.join(save_path, 'subject_metrics.csv'))
