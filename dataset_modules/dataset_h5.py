...
class Whole_Slide_Bag_FP(Dataset):
	...
	def __getitem__(self, idx):
		...
		with h5py.File(self.file_path,'r') as hdf5_file:
			coord = hdf5_file['coords'][idx]
		if tcga:
			img = self.wsi.read_region(coord, self.patch_level, (self.patch_size*2, self.patch_size*2)).convert('RGB')
			img = img.resize((self.patch_size, self.patch_size), Image.Resampling.LANCZOS)
		else: img = self.wsi.read_region(coord, self.patch_level, (self.patch_size, self.patch_size)).convert('RGB')
		img = self.roi_transforms(img)
		return {'img': img, 'coord': coord}
