...
def training_models(X_train, y_train, save_path):
    ...
    classifiers = [rf_classifier, xgb_classifier, nn] #Linear regression, random forest, XGB and MLP
    ...
    param_grid_nn = {
        'hidden_layer_sizes': [(100, 50, 25), (100, 50), (50, 50, 50)],
        'activation': ['relu'],
        'solver': ['adam', 'sgd'],
        'alpha': [0.0001, 0.001],
        'learning_rate': ['adaptive'],
        'max_iter' : [200, 600, 700]
    }
    ...
    for classifier in classifiers:  #for each classifier
        ...
        for fold, (train, val) in enumerate(cv.split(X_train, y_train)): #train with 10 fold CV the best estimator
            print("scripts model on {}-fold validation set".format(fold))
        ...
        voting_classifier = VotingClassifier(
            estimators=[(f"{classifier_name}_fold_{i}", clf) for i, clf in
                        enumerate(classifiers_dict[classifier_name])],
            voting='soft'
        )
        voting_classifiers[classifier_name] = voting_classifier
