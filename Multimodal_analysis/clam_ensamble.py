...
def ensamble_clam(path, model):
    ...
    pattern = re.compile(r'Probs_fold_\d+\.csv$')
    ...
    dest_path = os.path.join('probs_data/', model)
    os.makedirs(dest_path, exist_ok=True)

    mean_predictions.to_csv(os.path.join(dest_path, f'{model_size}_{gene}.csv'), index=False)
...
if __name__ == '__main__':
    ...
    gen_path = r'E:\Users\Berloco\PycharmProjects\CLAM\InferenceCPTACFull'

    for fold_name in os.listdir(gen_path):
        fold_path = os.path.join(gen_path, fold_name)
        model = fold_name.split('_')[-1]
        for subdir in os.listdir(fold_path):
            ensamble_clam(os.path.join(fold_path, subdir), model)