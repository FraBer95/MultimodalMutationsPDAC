...
def running_eval_AE(folder, test, features, label, results):
    ...
    metrics_df.to_csv(os.path.join(final_path, '{}_subject_metrics.csv'.format(dim)))
...
def running_eval_DEG(folder, test, features, label, results): #folder = DEG+Gene -> MODELLO
    ...
    for modelfolder in os.listdir(folder):
        ...
        for key, val in class_dict.items():
            ...
            predict_df = pd.concat([test['case_id'], test['new_case_id'], y_pred_df, pd.DataFrame(y_test[:, 0], columns=['Y']),
                                    pd.DataFrame(y_pred, columns=['Y_hat'])], axis=1)
...
if __name__ == '__main__':
    ...
    results_folder = r'../InferenceTCGARnaTranspose'
    ...
    for log_file in os.listdir(logs_path):
        if AE:
            ...
            for gene in os.listdir(path_to_dim):
                test = pd.read_csv( r'E:\Users\\Berloco\PycharmProjects\CLAM\genomic_analysis\datasets\classification_dataset\AE\{}\AE_{}.csv'.format(
                    dim, gene))
            ...
            df = pd.read_csv(r'E:\Users\\Berloco\PycharmProjects\CLAM\genomic_analysis\datasets\classification_dataset\DEG\DEG_{}.csv'.format(gene))
            ...
            #test['new_case_id'] = df['new_case_id']
