...
os.environ['CONCH_CKPT_PATH'] = "/mnt/e/Users/Berloco/PycharmProjects/CLAM/features_extractors/CONCH.bin"
os.environ['UNI_CKPT_PATH'] =  "/mnt/e/Users/Berloco/PycharmProjects/CLAM/features_extractors/UNI.bin"
...
if __name__ == '__main__':
    ...
    print("Skip decision input in line 100 of create_heatmaps.py...")
    ...
    ckpt_path_gen = model_args.ckpt_path
    print('\nckpt path: {}'.format(ckpt_path_gen))



    for ckpt in os.listdir(ckpt_path_gen):
        if ckpt.endswith('.pt'):
            folder = ckpt.split('_')[1]
            ckpt_path = os.path.join(ckpt_path_gen, ckpt)
            ...
            for i in tqdm(range(len(process_stack))):
                ...
                p_slide_save_dir = os.path.join(exp_args.production_save_dir, exp_args.save_exp_code, folder, str(grouping))
                os.makedirs(p_slide_save_dir, exist_ok=True)

                r_slide_save_dir = os.path.join(exp_args.raw_save_dir, exp_args.save_exp_code, folder, str(grouping),  slide_id)
                ...
                for sample in samples:
                    if sample['sample']:
                        ...
                        sample_save_dir =  os.path.join(exp_args.production_save_dir, exp_args.save_exp_code, folder, 'sampled_patches', str(tag), sample['name'])
                ...
                if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
                    ...
                    if folder == '9':
                        scores = scores_array / 9
                        print("Saving heatmap of 9 folder with average scores")
